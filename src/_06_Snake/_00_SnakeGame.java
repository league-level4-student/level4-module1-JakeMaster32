...
public class _00_SnakeGame implements ActionListener, KeyListener {
	...
	public void startGame() {
		...
		String instructions = "Move around the snake, and eat the apples to grow longer. Don't collide with yourself or the walls. ";
	...
	private void setFoodLocation() {
		...
		location = new Location(rand.nextInt(WIDTH), rand.nextInt(HEIGHT));
	...
	private void gameOver() {
		...
		String playAgain = JOptionPane.showInputDialog("Wanna play again? (yes or no");
...
setFoodLocation();
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (snake.isHeadCollidingWithBody()) {
			gameOver();
		} else if ((snake.getHeadLocation().x > WIDTH) || (snake.getHeadLocation().x < 0)) {
			gameOver();
		} else if ((snake.getHeadLocation().y > HEIGHT) || (snake.getHeadLocation().y < 0)) {
			gameOver();
		}
